...
interface AthleteProfile {
  ...
  swim: {
    ...
    cssPacePer100m?: Seconds;
    track: 'learn' | 'develop' | 'refine';
  };
...
**RPE → IR** (the fallback path, and the _only_ path for most first-timers):
...
Base split by race distance (share of weekly _time_, not load):
...
- Keep the same weekly _shape_ week to week — habit beats optimality.
